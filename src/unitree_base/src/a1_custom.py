...
os.chdir('/home/user/.local/share/ov/pkg/isaac_sim-2022.1.0')
print(f'changed current directory to {os.getcwd()}')
# os.system('gnome-terminal -x ./python.sh standalone_examples/api/omni.isaac.quadruped/a1_custom.py')
os.system('./python.sh standalone_examples/api/omni.isaac.quadruped/a1_custom.py')
...
class A1_direct_runner(object):
    def __init__(self, physics_dt, render_dt) -> None:
        ...
        self._sub_joint_cmd = rospy.Subscriber(
            "isaac_a1/joint_torque_cmd", geometry_msgs.Twist, self.joint_command_callback
        )
    ...
    def joint_command_callback(self, data):
        ...
        """
        # 0, 1 -> linear x, linear y
        # 5 -> angular z = rotate
        print('Callback functioned!!!')

        i = 0
        for idx in list([0, 1, 5]):   
            print(f'i {i} | idx {idx}')
            self._base_command[i] = data.effort[idx]
            i += 1
